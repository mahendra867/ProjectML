...
import numpy as np
import pandas as pd
from PROJECTML.pipeline.prediction import PredictionPipeline # here iam importing the predicition pipeline which it do predicts on the uploaded data by user and it return the prediction value
...
def index():
    if request.method == 'POST':
        try:
            ...
            data = [fixed_acidity,volatile_acidity,citric_acid,residual_sugar,chlorides,free_sulfur_dioxide,total_sulfur_dioxide,density,pH,sulphates,alcohol] # here iam taking the passed independent new values by user in the form of list
            data = np.array(data).reshape(1, 11) # here iam converting to numpy array of that data 
            
            obj = PredictionPipeline() # here iam initilizing the PredictionPipeline() and on the top of it calling the predic
            predict = obj.predict(data)

            return render_template('results.html', prediction = str(predict)) # here iam returning the prediction
...
if __name__ == "__main__":
	...
	app.run(host="0.0.0.0", port = 8080) # here host , port values are below as follows 
